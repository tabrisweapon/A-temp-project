...
def eval_adv_test_whitebox(model, device, test_loader):
    ...
    for data, target in test_loader:
        ...
        err_natural, err_robust, err_smth = _pgd_whitebox(model, X, y)
        robust_err_total += err_robust
        natural_err_total += err_natural
        smooth_err_total += err_smth
    print('natural_err_total: ', natural_err_total)
    print('robust_err_total: ', robust_err_total)
    print('smooth_err_total: ', smooth_err_total)
...
def main():
    ...
    if not args.black_attack:
        ...
        model = WideResNet(widen_factor=args.width, num_classes=100).to(device)
        # model = DenseNet3(depth=40, num_classes=100, growth_rate=args.width).to(device)
